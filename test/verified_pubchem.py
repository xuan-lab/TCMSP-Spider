...
# cas_id_col = 'CAS_id'        # No longer needed for cross-validation lookup step, but used in comparison map
pubchem_id_col = 'PubChem_id'  # 3rd priority for lookup

# Limit the number of subsequent aliases to try IF the first alias fails
# Total aliases tried might be 1 (if first succeeds) or up to 1 + max_subsequent_aliases_to_try
...
if __name__ == "__main__":
    ...
    # --- Validate essential lookup columns exist ---
    lookup_cols_needed = [name_col, alias_col, pubchem_id_col]
    missing_lookup_cols = [col for col in lookup_cols_needed if col not in df.columns]
    if missing_lookup_cols:
        print(f"错误：输入文件中缺少用于查找的必要列: {', '.join(missing_lookup_cols)}")
        exit()
    cas_id_col_internal = 'CAS_id' # Assuming standard name for check
    if cas_id_col_internal not in df.columns:
         print(f"警告: 输入文件中缺少列 '{cas_id_col_internal}'，CAS号比对将无法进行。")
    ...
    for index, row in df.iterrows():
        ...
        found_compound = None
        found_by = None
        # row_updated and row_error flags removed, use sets instead

        # --- Hierarchical Lookup (No External Cross-Validation, First Alias Priority) ---
            ...
            try:
                ...
                if len(cids) == 1:
                    ...
                    if potential_compound:
                        found_compound = potential_compound; found_by = name_col # Accept directly
                elif len(cids) > 1:
        ...
        # Step 2: If not found by Name, try finding by Alias (First alias gets priority) --- << MODIFIED >>
        if not found_compound and alias_col in df.columns:
            ...
            if not is_nan_or_none(alias_value) and isinstance(alias_value, str) and alias_value.strip() != '':
                ...
                if aliases:
                    ...
                    first_alias_success = False
                    try:
                        # print(f"行 {index + 2}: 尝试通过第一个别名 '{first_alias}' 查找...")
                        ...
                        if len(cids) == 1:
                            ...
                            if potential_compound:
                                ...
                                first_alias_success = True # Mark success
                        elif len(cids) > 1:
                        ...
                        # else: # len(cids) == 0
                        #     if overall_status == "未处理" or "未找到匹配" in overall_status: overall_status = f"第一个别名 '{first_alias}' 未找到匹配"
                    except Exception as e:
                        print(f"警告: 行 {index + 2}, 查找第一个别名 '{first_alias}' 时出错: {e}"); error_indices.add(index)

                    # --- If FIRST alias failed, try subsequent aliases with consistency check ---
                    if not first_alias_success and len(aliases) > 1:
                        ...
                        if aliases_to_attempt: # Check if there are subsequent aliases to try
                            ...
                            # Check consistency of results from subsequent aliases
                            if len(found_cids_from_aliases) == 1:
                                ...
                                if overall_status == "未处理" or "未找到匹配" in overall_status:
                                 ...
                                 # Append status about subsequent aliases
                    ...
                    elif not first_alias_success and len(aliases) <= 1:
                         if overall_status == "未处理" or "未找到匹配" in overall_status: overall_status = f"第一个别名 '{first_alias}' 未找到唯一匹配"
        ...
        if found_compound:
            ...
            row_updated_flag = False # Local flag for this row
            ...
            if found_by and (found_by == name_col or found_by.startswith(alias_col)):
                ...
                elif original_cid == found_cid: pass # IDs match
        ...
        # --- Step 5: Handle cases where no unique compound was found ---
            ...
            if "未找到" in overall_status and "多个匹配" not in overall_status:
    ...
    # print(f"查找被拒行数 (交叉验证失败): {cross_validation_rejects}") # Removed cross-validation
